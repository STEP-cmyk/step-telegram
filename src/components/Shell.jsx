...
import { applyTheme, getSystemTheme, initializeTheme } from '../lib/theme'
...
export default function Shell({children}){
  ...
  const nav = [
    { to:'/',              label: t('summary'),      icon: Home },
    { to:'/goals',         label: t('goals'),        icon: Target },
    { to:'/habits',        label: t('habits'),       icon: CheckSquare },
    { to:'/wishes',        label: t('wishes'),       icon: Heart },
    ...(data?.settings?.visibility?.notes !== false ? [{ to:'/notes', label: t('notes'), icon: NotebookText }] : []),
    ...(data?.settings?.visibility?.competitions !== false ? [{ to:'/competitions', label: t('competitions'), icon: Trophy }] : []),
  ]
  ...
  return (
    <div className="app-container min-h-dvh bg-gradient-to-br from-blue-50 via-white to-red-50 dark:from-zinc-900 dark:via-zinc-800 dark:to-orange-900/20 text-zinc-900 dark:text-zinc-50">
      ...
      <nav className="tabbar bg-white/95 dark:bg-zinc-900/95 backdrop-blur-sm border-t border-gray-200 dark:border-zinc-700">
